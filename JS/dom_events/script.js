...
const updateLength = (event, form) => {
  event.preventDefault()
  let backpackObject = backpackObjectArray.find(
    ({ id }) => id === form.parentElement.parentElement.parentElement.id
  );
  let textBox = form.querySelector("input")
  let newLength = textBox.value

  let lengthDisplay = form.parentElement.querySelector("span")
  lengthDisplay.innerText = `${newLength} inches`

  let side = form.parentElement.getAttribute("data-side")
  side === 'left' 
  ? backpackObject.strapLength.left = newLength 
...
const backpackList = backpackObjectArray.map((backpack) => {
  ...
  let forms = backpackArticle.querySelectorAll("form")
  
  // Add event listener
  forms.forEach(form =>{
    form.addEventListener("submit", (event)=>{
      updateLength(event, form)
    })
  })
